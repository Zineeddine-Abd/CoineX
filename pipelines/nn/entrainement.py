"""
Entraînement du CNN de comptage de pièces (à exécuter sur Kaggle/Colab avec GPU).
...
from pipelines.nn.pretraitement import pretraiter_image_brut, NUM_CHANNELS
...
def calculer_statistiques_dataset(preloaded_data):
    """
    Moyenne et écart-type par canal sur les données pré-chargées
    (non augmentées, non normalisées). Renvoie deux tenseurs (NUM_CHANNELS,).
    """
...
class CoinDataset(Dataset):
    def __init__(self, dossier_images, fichier_json, augment=False):
        ...
        # à chaque itération. Les augmentations s'appliquent à la volée.
        self.preloaded_data = []
    ...
    def _augmenter(self, x):
        ...
        if random.random() < 0.7:
            ...
            # Re-binariser les canaux binaires (3 Otsu, 4 Canny) après interpolation.
        ...
        # --- Photométrique (canaux continus 0-2) ---
        if random.random() < 0.5:
...
def entrainer_modele(
    ...
    json_train="data/train.json",
    dossier_val="data/validation",
    json_val="data/validation.json",
    epochs=200,
    batch_size=16,
    lr=1e-3,
    patience=30,
    seed=42,
    chemin_sauvegarde="meilleur_modele_nn.pth",
):
    # Reproductibilité
    random.seed(seed)
    np.random.seed(seed)
    ...
    train_dataset = CoinDataset(dossier_train, json_train, augment=True)
    val_dataset = CoinDataset(dossier_val, json_val, augment=False)

    # --- [2/5] Statistiques de normalisation (TRAIN uniquement) ---
    ...
    val_loader = DataLoader(
        val_dataset, batch_size=batch_size, shuffle=False, pin_memory=pin_mem, num_workers=2,
    )
    ...
    # Mixed precision : ~1.7x plus rapide sur T4
